<div align="center">

<img src="https://readme-typing-svg.herokuapp.com?lines=你好，我是yanghuaqing452👋;这里是我的个人主页(=^▽^=)&center=true&size=27&color=FFFFFF&background=111111&vCenter=true&width=500" />

<br/>

</div>

---

## 🧭 快速导航

> 点击直接跳转到对应项目仓库

| 🚀 项目 | 📌 简介 | 🏷️ 状态 |
|--------|--------|--------|
| [🔒 Project-A](https://github.com/yanghuaqing452/Project-A) | 🐍 Python 工具类 | 未公开 |
| [🔒 Project-B](https://github.com/yanghuaqing452/Project-B) | 🟢 Node.js 服务 | 未公开 |
| [🔒 Project-C](https://github.com/yanghuaqing452/Project-C) | 🐧 Linux 脚本集 | 未公开 |

<!-- 项目公开后把上方的 Project-A/B/C 换成真实仓库名即可 -->

---

## 📖 关于我

```python
class yanghuaqing452:
    def __init__(self):
        self.focus     = ["学习", "开发", "重构"]
        self.learning  = ["Python", "Node.js", "Linux"]  # 正在学 & 常用
        self.status    = "🔨 持续构建中..."
        self.motto     = "先让它跑起来，再让它跑得好"
```

---

## 🛠️ 技术栈

| 技术 | 用途 | 状态 |
|------|------|------|
| Python | 脚本 / 工具开发 | 🌱 学习中 |
| Node.js | 服务端开发 | 🌱 学习中 |
| Linux | 日常系统环境 | 🔧 常用 |
| Bash | 脚本 / 自动化 | 🔧 常用 |
| Git | 版本管理 | 🔧 常用 |

---

## 🚀 项目展示

<!-- ✅ 已公开项目：将下方注释替换为你的项目卡片 -->
<!--
[![项目名](https://github-readme-stats.vercel.app/api/pin/?username=yanghuaqing452&repo=你的仓库名&theme=tokyonight&hide_border=true)](https://github.com/yanghuaqing452/你的仓库名)
-->

> 📢 公开项目即将上线，敬请期待！



---

## 📊 GitHub 统计

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=yanghuaqing452&show_icons=true&theme=tokyonight&hide_border=true&locale=cn&count_private=true" height="165"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=yanghuaqing452&layout=compact&theme=tokyonight&hide_border=true&locale=cn" height="165"/>

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=yanghuaqing452&theme=tokyonight&hide_border=true&locale=zh_Hans" />

</div>

---

## 📫 联系我

<div align="center">

<a href="https://space.bilibili.com/354607179746094197" target="_blank">
  <img src="https://img.shields.io/badge/Bilibili-B站-fb7299?style=for-the-badge&logo=bilibili&logoColor=white&labelColor=2e2e2e" />
</a>
&nbsp;
<a href="mailto:scrape-fe@wqtiingqi.anonaddy.com">
  <img src="https://img.shields.io/badge/Email-邮箱-D44638?style=for-the-badge&logo=gmail&logoColor=white&labelColor=2e2e2e" />
</a>

</div>

---

<div align="center">
  <sub>📝 README 持续更新中 · 最后修改于 2026</sub>
</div>
